...
class Node4 { // 노드 클래스
    ...
    public Node4 next; // 다음 노드를 가리키는 포인터
...
class LinkedList4 { // 링크드 리스트 클래스
    ...
    public Node4 getNode(int index) {
        Node4 cur = this.head; // 현재 노드에 head 담기
    ...
    // index번째 노드를 제거하는 메서드
    public void deleteNode(int index) {
        Node4 nextNode = this.getNode(index + 1);

        // index가 0일 경우 nextNode를 head로 설정한다.
        if (index == 0) {
            head = nextNode;
        } else { // 그 외의 경우 index - 1번째 노드의 next를 index + 1번째 노드로 설정한다.
            Node4 prevNode = this.getNode(index - 1);
