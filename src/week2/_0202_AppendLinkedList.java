...
public class _0202_AppendLinkedList {
    public static void main(String[] args) {
        Node4 node4 = new Node4(3); // 노드 객체 생성 후 data애 3 넣기
        System.out.println(node4.data + " " + node4.next); // data는 3, next는 null로 출력

        LinkedList4 linkedList4 = new LinkedList4(5); // 링크드 리스트 객체 생성 후 5 넣기
        System.out.println(linkedList4.head.data); // 맨 앞 노드의 data인 5 출력

        linkedList4.append(12); // 값이 5인 head 노드 뒤에 값이 12인 노드 추가
        linkedList4.append(8); // 값이 12인 노드 뒤에 값이 8인 노드 추가
        linkedList4.printAll(); // 노드 모두 출력 : 5 12 8
    }
}
...
class LinkedList { // 링크드 리스트 클래스
    ...
    public LinkedList(int value) {
        this.head = new Node4(value); // 노드 객체에 value를 넣어 head 초기화
    }
